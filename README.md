...
| Livello | Nome          | Descrizione                          | Socket |
|---------|---------------|--------------------------------------|--------|
...
2. **`bind(IP, porta)`** - Associa socket a un indirizzo
...
4. **`accept()`** - Accetta connessione (**bloccante**)
   ...
   - Il *three-way handshake* è già completato:
...
#### **END OF FILE** (Chiusura)
7. **`close()`** - Chiude connessione client
8. **`close()`** - Chiude socket server
